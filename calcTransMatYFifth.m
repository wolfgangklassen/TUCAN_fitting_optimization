...
%Feb 2020: checked all terms up to order 5 with Maedeh Lavvaf, corrected a
%bunch of typos.  Check was done both by hand and using Jeffs Pis.py
%program.
x = pos(:,1);
y = pos(:,2);
z = pos(:,3);
...
transMat(:,29) = z.^4 - (3/2).*(x.^2).*(z.^2) - (9/2).*(y.^2).*(z.^2) + ...
    0.125.*(x.^4) + 0.75.*(x.^2).*(y.^2) + (5/8).*(y.^4);%
transMat(:,30) = y.*z.*(1.5.*(x.^2 + y.^2) - 2.*(z.^2));%
transMat(:,31) = x.*y.*(0.5.*(x.^2 + y.^2) - 3.*(z.^2));%
...
transMat(:,38) = 10.*(x.^3).*(z.^2) - 30.*x.*(y.^2).*(z.^2) +...
    5.*x.*(y.^4) - x.^5;%
transMat(:,39) = 10.*(x.^2).*(z.^3) - (15/4).*(x.^4).*z - ...
    (15/2).*(x.^2).*(y.^2).*z - 10.*(y.^2).*(z.^3) + (25/4).*(y.^4).*z;%
transMat(:,40) = 5.*x.*(z.^4) - 5.*(x.^3).*(z.^2) - ...
    15.*x.*(y.^2).*(z.^2) + (5/16).*(x.^5) + (25/16).*x.*(y.^4) + ...
    (15/8).*(x.^3).*(y.^2);%
transMat(:,41) = z.^5 - 2.5.*(x.^2).*(z.^3) - (15/2).*(y.^2).*(z.^3) + ...
    (5/8).*(x.^4).*z + (25/8).*(y.^4).*z + (15/4).*(x.^2).*(y.^2).*z;%
transMat(:,42) = -2.5.*y.*(z.^4) + (15/4).*(y.^3).*(z.^2) + ...
    (15/4).*(x.^2).*y.*(z.^2) - (5/16).*(x.^4).*y - ...
    (5/8).*(x.^2).*(y.^3) - (5/16).*(y.^5);%
transMat(:,43) = -5.*x.*y.*(z.^3) + (5/2).*x.*(y.^3).*z + ...
    (5/2).*(x.^3).*y.*z;%
transMat(:,44) = -(5/8).*(x.^2).*(y.^3) + (5/16).*(x.^4).*y - ...
    5.*y.*(z.^4) + 10.*(y.^3).*(z.^2) - (15/16).*(y.^5);%
transMat(:,45) = 5.*x.*y.*z.*(x.^2 + 3.*(y.^2) - 4.*(z.^2));%
transMat(:,46) = 10.*(y.^3).*(z.^2) + 5.*(x.^2).*(y.^3) + ...
    (5/2).*(x.^4).*y - 1.5.*(y.^5) - 30.*(x.^2).*y.*(z.^2);%
