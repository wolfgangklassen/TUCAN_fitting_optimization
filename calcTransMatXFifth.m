...
transMat(:,29) = 0.5.*x.*y.*(x.^2 + y.^2 - 6.*(z.^2));%
transMat(:,30) = x.*z.*(1.5.*(y.^2 + x.^2) - 2.*(z.^2));%
transMat(:,31) = z.^4 - (9/2).*(x.^2).*(z.^2) - (3/2).*(y.^2).*(z.^2) + ...
    0.75.*(x.^2).*(y.^2) + 0.125.*y.^4 + (5/8).*x.^4;%
...
transMat(:,36) = 5.*(x.^4).*y - 10.*(x.^2).*(y.^3) + y.^5;%
transMat(:,37) = 20.*x.*y.*z.*(x.^2 - y.^2);%
transMat(:,38) = 30.*(x.^2).*y.*(z.^2) - 5.*(x.^4).*y -...
    10.*(y.^3).*(z.^2) + y.^5;%
transMat(:,39) = 5.*x.*y.*z.*(4.*(z.^2) - 3.*x.^2 - (y.^2));%
transMat(:,40) = 5.*y.*(z.^4) - 15.*(x.^2).*y.*(z.^2) - ...
    5.*(y.^3).*(z.^2) + (25/16).*(x.^4).*y + (5/16).*(y.^5) + ...
    (15/8).*(x.^2).*(y.^3);%
transMat(:,41) = (5/2).*x.*y.*z.*(x.^2 + y.^2 - 2.*(z.^2));%
transMat(:,42) = -(5/2).*x.*(z.^4) + (15/4).*(x.^3).*(z.^2) + ...
    (15/4).*x.*(y.^2).*(z.^2) - (5/16).*(x.^5) - ...
    (5/8).*(x.^3).*(y.^2) - (5/16).*x.*(y.^4);%
transMat(:,43) = (z.^5) - (15/2).*(x.^2).*(z.^3) - ...
    (5/2).*(y.^2).*(z.^3) + (25/8).*(x.^4).*z + (5/8).*(y.^4).*z + ...
    (15/4).*(x.^2).*(y.^2).*z;%
transMat(:,44) = 5.*x.*(z.^4) - 10.*(x.^3).*(z.^2) + (15/16).*x.^5 - ...
